...
void loop()
{
    ...
    for (int j = 0; j < 5; j++) {
        ...
        //finished with one entire reading, wait 9 ms, do until 5 readings
        delay(9);
    }
    /*TODO: check thermistor values */
    for (int i = 0; i < 5; i++) {
        if (checkThermistor(10000, th[i]) >= 100) {
                msg[1] = 2;
        } else if (checkThermistor(10000, th[i]) >= 80) {
            if (msg[1] == 0) {
                msg[1] = 1;
            }
        }
    ...
    int firstButton = analogRead(A2);
   
    int nextButton = analogRead(A3);
    
    Serial.println(firstButton);
      Serial.println(nextButton);
    
    if(firstButton > 500 || nextButton >500) {
      msg[3] = 1;
